...
function InteractiveManDown:OnSoldierEntityData(p_Instance)
	...
	local s_SoldierBlueprint = SoldierBlueprint(ResourceManager:FindInstanceByGuid(Guid('F256E142-C9D8-4BFE-985B-3960B9E9D189'), Guid('261E43BF-259B-41D2-BF3B-9AE4DDA96AD2')))
	s_SoldierBlueprint:MakeWritable()
	
	for i=#s_SoldierBlueprint.eventConnections, 1, -1 do
		if s_SoldierBlueprint.eventConnections[i].source:Is('SoldierEntityData') and s_SoldierBlueprint.eventConnections[i].target:Is('PlayerFilterEntityData') then
	...
	for i=#s_SoldierBlueprint.propertyConnections, 1, -1 do
		if s_SoldierBlueprint.propertyConnections[i].source.instanceGuid == Guid('9DF212F6-73C1-4218-9110-2090EE95F730') or s_SoldierBlueprint.propertyConnections[i].target.instanceGuid == Guid('9DF212F6-73C1-4218-9110-2090EE95F730') then
	...
	local s_EntityInteractionComponentData = EntityInteractionComponentData(ResourceManager:FindInstanceByGuid(Guid('F256E142-C9D8-4BFE-985B-3960B9E9D189'), Guid('9C51D42E-94F9-424A-89D2-CBBCA32F1BCE')))
	local s_InterfaceDescriptorData = InterfaceDescriptorData(ResourceManager:FindInstanceByGuid(Guid('F256E142-C9D8-4BFE-985B-3960B9E9D189'), Guid('9C158C06-AFDA-4CE5-8323-F41D356B2971')))
	
	local onSoldierInteractionFinishedConnection = self.m_EventConnections:Create(s_EntityInteractionComponentData, s_InterfaceDescriptorData, -1956653754, -1956653754, 3)
	local onSoldierInteractionStarted = self.m_EventConnections:Create(s_EntityInteractionComponentData, s_InterfaceDescriptorData, 1783953429, 1783953429, 3)
	local onSoldierInteractionCancelled = self.m_EventConnections:Create(s_EntityInteractionComponentData, s_InterfaceDescriptorData, -1947428449, -1947428449, 3)
	local onInteractionStopped = self.m_EventConnections:Create(s_EntityInteractionComponentData, s_InterfaceDescriptorData, 1565407255, 1565407255, 3)
	local onInteractionStarted = self.m_EventConnections:Create(s_EntityInteractionComponentData, s_InterfaceDescriptorData, 1572599007, 1572599007, 3)
	...
	local onBeingInteractedStarted = self.m_EventConnections:Create(p_Instance, s_InterfaceDescriptorData, -1741104687, -1741104687, 3)
	local onBeingInteractedCancelled = self.m_EventConnections:Create(p_Instance, s_InterfaceDescriptorData, -1025749669, -1025749669, 3)
	local onBeingInteractedFinished = self.m_EventConnections:Create(p_Instance, s_InterfaceDescriptorData, 1957374978, 1957374978, 3)
	local onBeingInteractedFinishedToReviveConnection = self.m_EventConnections:Create(p_Instance, p_Instance, 1957374978, -1001523010, 3)
...
function InteractiveManDown:CreateManDownCustomizeSoldierData()
	...
	s_UnlockWeaponAndSlot.weapon = SoldierWeaponUnlockAsset(ResourceManager:FindInstanceByGuid(Guid('7C58AA2F-DCF2-4206-8880-E32497C15218'), Guid('B145A444-BC4D-48BF-806A-0CEFA0EC231B')))
