...
function SpectatorCamera:OnPlayerKilled(p_Player)
    ...
    elseif self.m_SpectatedPlayer == nil or p_Player.id == self.m_SpectatedPlayer.id then
...
function SpectatorCamera:OnClientUpdateInput()
    if self:IsEnabled() then
        if InputManager:WentKeyDown(InputDeviceKeys.IDK_Space) or
            InputManager:WentKeyDown(InputDeviceKeys.IDK_ArrowRight) then
            self:SpectateNextPlayer()
        end
...
function SpectatorCamera:FindFirstPlayerToSpectate(p_OnlySquadMates)
    ...
    if p_OnlySquadMates then
        if s_LocalPlayer.squadId == SquadId.SquadNone then
            self:FindFirstPlayerToSpectate(false)
        end
...
function SpectatorCamera:OnEngineUpdate(p_DeltaTime)
    ...
    local s_Yaw =
        MathHelper:LerpRadians(self.m_LastYaw, self.m_PlayersPitchAndYaw[s_Player.id]["Yaw"], p_DeltaTime * 10)
    ...
    local s_Hit = RaycastManager:Raycast(self.m_LookAtPos, s_CameraLocation, RayCastFlags.DontCheckWater |
                                             RayCastFlags.DontCheckCharacter | RayCastFlags.DontCheckRagdoll)
